<?php include("header.php") ?>
<main class="main">
    <section class="main-swiper-section">
        <div class="swiper main-swiper">
            <div class="swiper-wrapper">
                <div class="swiper-slide">
                    <div class="swiper-holder">
                        <div class="text">
                            <h2 class="big">NOVÁ KOLA <br>
                                OD TREKU 2022</h2>
                            <p class="big">Připravily jsme 16 nových kol od firmy Trek </p>
                            <a href="" class="btn full-orange">
                                <span>zobrazit produkty</span>
                            </a>
                        </div>
                        <img src="data/images/swiper-main.jpg" alt="" />
                    </div>
                </div>
                <div class="swiper-slide">
                    <div class="swiper-holder">
                        <div class="text">
                            <h2 class="big">NOVÁ KOLA <br>
                                OD TREKU 2022</h2>
                            <p class="big">Připravily jsme 16 nových kol od firmy Trek </p>
                            <a href="" class="btn full-orange">
                                <span>zobrazit produkty</span>
                            </a>
                        </div>
                        <img src="data/images/swiper-main.jpg" alt="" />
                    </div>
                </div>
                <div class="swiper-slide">
                    <div class="swiper-holder">
                        <div class="text">
                            <h2 class="big">NOVÁ KOLA <br>
                                OD TREKU 2022</h2>
                            <p class="big">Připravily jsme 16 nových kol od firmy Trek </p>
                            <a href="" class="btn full-orange">
                                <span>zobrazit produkty</span>
                            </a>
                        </div>
                        <img src="data/images/swiper-main.jpg" alt="" />
                    </div>
                </div>
            </div>
            <div class="nav-wrapper">
                <div class="swiper-pagination main-pagination"></div>

                <div class="swiper-button-prev swiper-button-prev-main"></div>
                <div class="swiper-button-next swiper-button-next-main"></div>
            </div>
        </div>
    </section>
    <section class="bikes-swiper">
        <div class="container">
            <h2 class="mark">Kola skladem</h2>
            <div class="prod-nav-wrapper">
                <div class="swiper-button-prev prod-prev"></div>
                <div class="swiper-button-next prod-next"></div>
            </div>
            <div class="swiper prod-swiper">

                <div class="swiper-wrapper">
                    <div class="swiper-slide">
                        <div class="swiper-holder">
                            <a href="" class="compare">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M13.4734 9.24167C13.5941 9.36238 13.7579 9.43019 13.9286 9.43019C14.0993 9.43019 14.263 9.36238 14.3837 9.24167C14.5044 9.12096 14.5722 8.95724 14.5722 8.78652C14.5722 8.61581 14.5044 8.45209 14.3837 8.33138L12.909 6.85795H15.2143C15.7258 6.85795 16.2163 7.06114 16.578 7.42282C16.9397 7.7845 17.1429 8.27504 17.1429 8.78652V14.6365C16.3624 14.7958 15.6689 15.2393 15.1968 15.8808C14.7247 16.5224 14.5076 17.3164 14.5877 18.1089C14.6678 18.9014 15.0392 19.636 15.6301 20.1702C16.221 20.7044 16.9892 21.0002 17.7857 21.0002C18.5823 21.0002 19.3504 20.7044 19.9413 20.1702C20.5322 19.636 20.9037 18.9014 20.9837 18.1089C21.0638 17.3164 20.8467 16.5224 20.3746 15.8808C19.9025 15.2393 19.209 14.7958 18.4286 14.6365V8.78652C18.4286 7.93404 18.0899 7.11648 17.4871 6.51368C16.8843 5.91089 16.0668 5.57224 15.2143 5.57224H12.909L14.3837 4.09881C14.5044 3.9781 14.5722 3.81438 14.5722 3.64367C14.5722 3.47296 14.5044 3.30924 14.3837 3.18853C14.263 3.06782 14.0993 3 13.9286 3C13.7579 3 13.5941 3.06782 13.4734 3.18853L10.902 5.75995C10.8421 5.81967 10.7946 5.89061 10.7622 5.96871C10.7298 6.04681 10.7131 6.13054 10.7131 6.2151C10.7131 6.29965 10.7298 6.38338 10.7622 6.46148C10.7946 6.53958 10.8421 6.61052 10.902 6.67024L13.4734 9.24167ZM19.7143 17.7865C19.7143 18.0398 19.6644 18.2906 19.5675 18.5246C19.4706 18.7585 19.3285 18.9711 19.1494 19.1502C18.9703 19.3293 18.7577 19.4714 18.5237 19.5683C18.2898 19.6652 18.039 19.7151 17.7857 19.7151C17.5324 19.7151 17.2817 19.6652 17.0477 19.5683C16.8137 19.4714 16.6011 19.3293 16.422 19.1502C16.2429 18.9711 16.1009 18.7585 16.0039 18.5246C15.907 18.2906 15.8571 18.0398 15.8571 17.7865C15.8571 17.275 16.0603 16.7845 16.422 16.4228C16.7837 16.0611 17.2742 15.858 17.7857 15.858C18.2972 15.858 18.7877 16.0611 19.1494 16.4228C19.5111 16.7845 19.7143 17.275 19.7143 17.7865ZM9.42857 6.2151C9.42872 6.95609 9.17285 7.67437 8.70427 8.24839C8.23568 8.82241 7.58316 9.2169 6.85714 9.3651V15.2151C6.85714 15.7266 7.06033 16.2171 7.42201 16.5788C7.78368 16.9405 8.27422 17.1437 8.78571 17.1437H11.091L9.61628 15.6702C9.49557 15.5495 9.42776 15.3858 9.42776 15.2151C9.42776 15.0444 9.49557 14.8807 9.61628 14.76C9.73699 14.6392 9.90071 14.5714 10.0714 14.5714C10.2421 14.5714 10.4059 14.6392 10.5266 14.76L13.098 17.3314C13.1579 17.3911 13.2054 17.462 13.2378 17.5401C13.2702 17.6182 13.2869 17.702 13.2869 17.7865C13.2869 17.8711 13.2702 17.9548 13.2378 18.0329C13.2054 18.111 13.1579 18.1819 13.098 18.2417L10.5266 20.8131C10.4059 20.9338 10.2421 21.0016 10.0714 21.0016C9.90071 21.0016 9.73699 20.9338 9.61628 20.8131C9.49557 20.6924 9.42776 20.5287 9.42776 20.358C9.42776 20.1872 9.49557 20.0235 9.61628 19.9028L11.091 18.4294H8.78571C7.93323 18.4294 7.11567 18.0907 6.51287 17.4879C5.91007 16.8851 5.57143 16.0676 5.57143 15.2151V9.3651C4.97555 9.24346 4.42641 8.95528 3.98778 8.53401C3.54916 8.11274 3.23904 7.57568 3.09346 6.9852C2.94787 6.39472 2.97279 5.77506 3.16532 5.19818C3.35786 4.62129 3.7101 4.11087 4.18114 3.72619C4.65218 3.3415 5.22269 3.09835 5.82641 3.02496C6.43012 2.95157 7.04228 3.05096 7.59176 3.31159C8.14125 3.57221 8.60552 3.98338 8.93065 4.49733C9.25579 5.01129 9.42844 5.60693 9.42857 6.2151ZM8.14286 6.2151C8.14286 5.96183 8.09297 5.71105 7.99605 5.47706C7.89913 5.24308 7.75707 5.03048 7.57799 4.85139C7.39891 4.67231 7.1863 4.53025 6.95232 4.43333C6.71833 4.33641 6.46755 4.28653 6.21428 4.28653C5.96102 4.28653 5.71024 4.33641 5.47625 4.43333C5.24227 4.53025 5.02966 4.67231 4.85058 4.85139C4.6715 5.03048 4.52944 5.24308 4.43252 5.47706C4.3356 5.71105 4.28571 5.96183 4.28571 6.2151C4.28571 6.72659 4.4889 7.21713 4.85058 7.5788C5.21226 7.94048 5.7028 8.14367 6.21428 8.14367C6.72577 8.14367 7.21631 7.94048 7.57799 7.5788C7.93967 7.21713 8.14286 6.72659 8.14286 6.2151Z" fill="black" stroke="black" stroke-width="0.5" />
                                </svg>
                            </a>

                            <div class="img">
                                <img src="data/images/bike-1.jpg" alt="">
                                <!-- <div class="perks">
                                        <span class="perk new">Novinka</span>
                                        <span class="perk best">Bestseller</span>
                                        <span class="perk sale">-50%</span>
                                    </div> -->
                                <div class="hover">
                                    <a href="" class="btn full-orange add-cart"><span>Do košíku</span></a>
                                    <a href="" class="btn border-white"><span>Detail</span></a>
                                    <ul>
                                        <li>XS</li>
                                        <li>S</li>
                                        <li>M</li>
                                        <li>L</li>
                                        <li>XL</li>
                                        <li>XXL</li>
                                    </ul>
                                </div>
                            </div>
                            <div class="helper">
                                <div class="upper-text">
                                    <h3 class="small">SuperCaliber 99</h3>
                                    <span class="stock in">Skladem</span>
                                </div>
                                <div class="lower-text">
                                    <span class="old-cost">45 950 Kč</span>
                                    <span class="final-price">32 850 Kč</span>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="swiper-holder">
                            <a href="" class="compare">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M13.4734 9.24167C13.5941 9.36238 13.7579 9.43019 13.9286 9.43019C14.0993 9.43019 14.263 9.36238 14.3837 9.24167C14.5044 9.12096 14.5722 8.95724 14.5722 8.78652C14.5722 8.61581 14.5044 8.45209 14.3837 8.33138L12.909 6.85795H15.2143C15.7258 6.85795 16.2163 7.06114 16.578 7.42282C16.9397 7.7845 17.1429 8.27504 17.1429 8.78652V14.6365C16.3624 14.7958 15.6689 15.2393 15.1968 15.8808C14.7247 16.5224 14.5076 17.3164 14.5877 18.1089C14.6678 18.9014 15.0392 19.636 15.6301 20.1702C16.221 20.7044 16.9892 21.0002 17.7857 21.0002C18.5823 21.0002 19.3504 20.7044 19.9413 20.1702C20.5322 19.636 20.9037 18.9014 20.9837 18.1089C21.0638 17.3164 20.8467 16.5224 20.3746 15.8808C19.9025 15.2393 19.209 14.7958 18.4286 14.6365V8.78652C18.4286 7.93404 18.0899 7.11648 17.4871 6.51368C16.8843 5.91089 16.0668 5.57224 15.2143 5.57224H12.909L14.3837 4.09881C14.5044 3.9781 14.5722 3.81438 14.5722 3.64367C14.5722 3.47296 14.5044 3.30924 14.3837 3.18853C14.263 3.06782 14.0993 3 13.9286 3C13.7579 3 13.5941 3.06782 13.4734 3.18853L10.902 5.75995C10.8421 5.81967 10.7946 5.89061 10.7622 5.96871C10.7298 6.04681 10.7131 6.13054 10.7131 6.2151C10.7131 6.29965 10.7298 6.38338 10.7622 6.46148C10.7946 6.53958 10.8421 6.61052 10.902 6.67024L13.4734 9.24167ZM19.7143 17.7865C19.7143 18.0398 19.6644 18.2906 19.5675 18.5246C19.4706 18.7585 19.3285 18.9711 19.1494 19.1502C18.9703 19.3293 18.7577 19.4714 18.5237 19.5683C18.2898 19.6652 18.039 19.7151 17.7857 19.7151C17.5324 19.7151 17.2817 19.6652 17.0477 19.5683C16.8137 19.4714 16.6011 19.3293 16.422 19.1502C16.2429 18.9711 16.1009 18.7585 16.0039 18.5246C15.907 18.2906 15.8571 18.0398 15.8571 17.7865C15.8571 17.275 16.0603 16.7845 16.422 16.4228C16.7837 16.0611 17.2742 15.858 17.7857 15.858C18.2972 15.858 18.7877 16.0611 19.1494 16.4228C19.5111 16.7845 19.7143 17.275 19.7143 17.7865ZM9.42857 6.2151C9.42872 6.95609 9.17285 7.67437 8.70427 8.24839C8.23568 8.82241 7.58316 9.2169 6.85714 9.3651V15.2151C6.85714 15.7266 7.06033 16.2171 7.42201 16.5788C7.78368 16.9405 8.27422 17.1437 8.78571 17.1437H11.091L9.61628 15.6702C9.49557 15.5495 9.42776 15.3858 9.42776 15.2151C9.42776 15.0444 9.49557 14.8807 9.61628 14.76C9.73699 14.6392 9.90071 14.5714 10.0714 14.5714C10.2421 14.5714 10.4059 14.6392 10.5266 14.76L13.098 17.3314C13.1579 17.3911 13.2054 17.462 13.2378 17.5401C13.2702 17.6182 13.2869 17.702 13.2869 17.7865C13.2869 17.8711 13.2702 17.9548 13.2378 18.0329C13.2054 18.111 13.1579 18.1819 13.098 18.2417L10.5266 20.8131C10.4059 20.9338 10.2421 21.0016 10.0714 21.0016C9.90071 21.0016 9.73699 20.9338 9.61628 20.8131C9.49557 20.6924 9.42776 20.5287 9.42776 20.358C9.42776 20.1872 9.49557 20.0235 9.61628 19.9028L11.091 18.4294H8.78571C7.93323 18.4294 7.11567 18.0907 6.51287 17.4879C5.91007 16.8851 5.57143 16.0676 5.57143 15.2151V9.3651C4.97555 9.24346 4.42641 8.95528 3.98778 8.53401C3.54916 8.11274 3.23904 7.57568 3.09346 6.9852C2.94787 6.39472 2.97279 5.77506 3.16532 5.19818C3.35786 4.62129 3.7101 4.11087 4.18114 3.72619C4.65218 3.3415 5.22269 3.09835 5.82641 3.02496C6.43012 2.95157 7.04228 3.05096 7.59176 3.31159C8.14125 3.57221 8.60552 3.98338 8.93065 4.49733C9.25579 5.01129 9.42844 5.60693 9.42857 6.2151ZM8.14286 6.2151C8.14286 5.96183 8.09297 5.71105 7.99605 5.47706C7.89913 5.24308 7.75707 5.03048 7.57799 4.85139C7.39891 4.67231 7.1863 4.53025 6.95232 4.43333C6.71833 4.33641 6.46755 4.28653 6.21428 4.28653C5.96102 4.28653 5.71024 4.33641 5.47625 4.43333C5.24227 4.53025 5.02966 4.67231 4.85058 4.85139C4.6715 5.03048 4.52944 5.24308 4.43252 5.47706C4.3356 5.71105 4.28571 5.96183 4.28571 6.2151C4.28571 6.72659 4.4889 7.21713 4.85058 7.5788C5.21226 7.94048 5.7028 8.14367 6.21428 8.14367C6.72577 8.14367 7.21631 7.94048 7.57799 7.5788C7.93967 7.21713 8.14286 6.72659 8.14286 6.2151Z" fill="black" stroke="black" stroke-width="0.5" />
                                </svg>
                            </a>

                            <div class="img">
                                <img src="data/images/bike-2.jpg" alt="">
                                <!-- <div class="perks">
                                        <span class="perk new">Novinka</span>
                                        <span class="perk best">Bestseller</span>
                                        <span class="perk sale">-50%</span>
                                    </div> -->
                                <div class="hover">
                                    <a href="" class="btn full-orange add-cart"><span>Do košíku</span></a>
                                    <a href="" class="btn border-white"><span>Detail</span></a>
                                    <ul>
                                        <li>XS</li>
                                        <li>S</li>
                                        <li>M</li>
                                        <li>L</li>
                                        <li>XL</li>
                                        <li>XXL</li>
                                    </ul>
                                </div>
                            </div>
                            <div class="helper">
                                <div class="upper-text">
                                    <h3 class="small">Slash 99XTR</h3>
                                    <span class="stock in">Skladem</span>
                                </div>
                                <div class="lower-text">
                                    <span class="old-cost">45 950 Kč</span>
                                    <span class="final-price">32 850 Kč</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="swiper-holder">
                            <a href="" class="compare">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M13.4734 9.24167C13.5941 9.36238 13.7579 9.43019 13.9286 9.43019C14.0993 9.43019 14.263 9.36238 14.3837 9.24167C14.5044 9.12096 14.5722 8.95724 14.5722 8.78652C14.5722 8.61581 14.5044 8.45209 14.3837 8.33138L12.909 6.85795H15.2143C15.7258 6.85795 16.2163 7.06114 16.578 7.42282C16.9397 7.7845 17.1429 8.27504 17.1429 8.78652V14.6365C16.3624 14.7958 15.6689 15.2393 15.1968 15.8808C14.7247 16.5224 14.5076 17.3164 14.5877 18.1089C14.6678 18.9014 15.0392 19.636 15.6301 20.1702C16.221 20.7044 16.9892 21.0002 17.7857 21.0002C18.5823 21.0002 19.3504 20.7044 19.9413 20.1702C20.5322 19.636 20.9037 18.9014 20.9837 18.1089C21.0638 17.3164 20.8467 16.5224 20.3746 15.8808C19.9025 15.2393 19.209 14.7958 18.4286 14.6365V8.78652C18.4286 7.93404 18.0899 7.11648 17.4871 6.51368C16.8843 5.91089 16.0668 5.57224 15.2143 5.57224H12.909L14.3837 4.09881C14.5044 3.9781 14.5722 3.81438 14.5722 3.64367C14.5722 3.47296 14.5044 3.30924 14.3837 3.18853C14.263 3.06782 14.0993 3 13.9286 3C13.7579 3 13.5941 3.06782 13.4734 3.18853L10.902 5.75995C10.8421 5.81967 10.7946 5.89061 10.7622 5.96871C10.7298 6.04681 10.7131 6.13054 10.7131 6.2151C10.7131 6.29965 10.7298 6.38338 10.7622 6.46148C10.7946 6.53958 10.8421 6.61052 10.902 6.67024L13.4734 9.24167ZM19.7143 17.7865C19.7143 18.0398 19.6644 18.2906 19.5675 18.5246C19.4706 18.7585 19.3285 18.9711 19.1494 19.1502C18.9703 19.3293 18.7577 19.4714 18.5237 19.5683C18.2898 19.6652 18.039 19.7151 17.7857 19.7151C17.5324 19.7151 17.2817 19.6652 17.0477 19.5683C16.8137 19.4714 16.6011 19.3293 16.422 19.1502C16.2429 18.9711 16.1009 18.7585 16.0039 18.5246C15.907 18.2906 15.8571 18.0398 15.8571 17.7865C15.8571 17.275 16.0603 16.7845 16.422 16.4228C16.7837 16.0611 17.2742 15.858 17.7857 15.858C18.2972 15.858 18.7877 16.0611 19.1494 16.4228C19.5111 16.7845 19.7143 17.275 19.7143 17.7865ZM9.42857 6.2151C9.42872 6.95609 9.17285 7.67437 8.70427 8.24839C8.23568 8.82241 7.58316 9.2169 6.85714 9.3651V15.2151C6.85714 15.7266 7.06033 16.2171 7.42201 16.5788C7.78368 16.9405 8.27422 17.1437 8.78571 17.1437H11.091L9.61628 15.6702C9.49557 15.5495 9.42776 15.3858 9.42776 15.2151C9.42776 15.0444 9.49557 14.8807 9.61628 14.76C9.73699 14.6392 9.90071 14.5714 10.0714 14.5714C10.2421 14.5714 10.4059 14.6392 10.5266 14.76L13.098 17.3314C13.1579 17.3911 13.2054 17.462 13.2378 17.5401C13.2702 17.6182 13.2869 17.702 13.2869 17.7865C13.2869 17.8711 13.2702 17.9548 13.2378 18.0329C13.2054 18.111 13.1579 18.1819 13.098 18.2417L10.5266 20.8131C10.4059 20.9338 10.2421 21.0016 10.0714 21.0016C9.90071 21.0016 9.73699 20.9338 9.61628 20.8131C9.49557 20.6924 9.42776 20.5287 9.42776 20.358C9.42776 20.1872 9.49557 20.0235 9.61628 19.9028L11.091 18.4294H8.78571C7.93323 18.4294 7.11567 18.0907 6.51287 17.4879C5.91007 16.8851 5.57143 16.0676 5.57143 15.2151V9.3651C4.97555 9.24346 4.42641 8.95528 3.98778 8.53401C3.54916 8.11274 3.23904 7.57568 3.09346 6.9852C2.94787 6.39472 2.97279 5.77506 3.16532 5.19818C3.35786 4.62129 3.7101 4.11087 4.18114 3.72619C4.65218 3.3415 5.22269 3.09835 5.82641 3.02496C6.43012 2.95157 7.04228 3.05096 7.59176 3.31159C8.14125 3.57221 8.60552 3.98338 8.93065 4.49733C9.25579 5.01129 9.42844 5.60693 9.42857 6.2151ZM8.14286 6.2151C8.14286 5.96183 8.09297 5.71105 7.99605 5.47706C7.89913 5.24308 7.75707 5.03048 7.57799 4.85139C7.39891 4.67231 7.1863 4.53025 6.95232 4.43333C6.71833 4.33641 6.46755 4.28653 6.21428 4.28653C5.96102 4.28653 5.71024 4.33641 5.47625 4.43333C5.24227 4.53025 5.02966 4.67231 4.85058 4.85139C4.6715 5.03048 4.52944 5.24308 4.43252 5.47706C4.3356 5.71105 4.28571 5.96183 4.28571 6.2151C4.28571 6.72659 4.4889 7.21713 4.85058 7.5788C5.21226 7.94048 5.7028 8.14367 6.21428 8.14367C6.72577 8.14367 7.21631 7.94048 7.57799 7.5788C7.93967 7.21713 8.14286 6.72659 8.14286 6.2151Z" fill="black" stroke="black" stroke-width="0.5" />
                                </svg>
                            </a>

                            <div class="img">
                                <img src="data/images/bike-3.jpg" alt="">
                                <!-- <div class="perks">
                                        <span class="perk new">Novinka</span>
                                        <span class="perk best">Bestseller</span>
                                        <span class="perk sale">-50%</span>
                                    </div> -->
                                <div class="hover">
                                    <a href="" class="btn full-orange add-cart"><span>Do košíku</span></a>
                                    <a href="" class="btn border-white"><span>Detail</span></a>
                                    <ul>
                                        <li>XS</li>
                                        <li>S</li>
                                        <li>M</li>
                                        <li>L</li>
                                        <li>XL</li>
                                        <li>XXL</li>
                                    </ul>
                                </div>
                            </div>
                            <div class="helper">
                                <div class="upper-text">
                                    <h3 class="small">TopFuel 99</h3>
                                    <span class="stock in">Skladem</span>
                                </div>
                                <div class="lower-text">
                                    <span class="old-cost">45 950 Kč</span>
                                    <span class="final-price">32 850 Kč</span>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="swiper-holder">
                            <a href="" class="compare">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M13.4734 9.24167C13.5941 9.36238 13.7579 9.43019 13.9286 9.43019C14.0993 9.43019 14.263 9.36238 14.3837 9.24167C14.5044 9.12096 14.5722 8.95724 14.5722 8.78652C14.5722 8.61581 14.5044 8.45209 14.3837 8.33138L12.909 6.85795H15.2143C15.7258 6.85795 16.2163 7.06114 16.578 7.42282C16.9397 7.7845 17.1429 8.27504 17.1429 8.78652V14.6365C16.3624 14.7958 15.6689 15.2393 15.1968 15.8808C14.7247 16.5224 14.5076 17.3164 14.5877 18.1089C14.6678 18.9014 15.0392 19.636 15.6301 20.1702C16.221 20.7044 16.9892 21.0002 17.7857 21.0002C18.5823 21.0002 19.3504 20.7044 19.9413 20.1702C20.5322 19.636 20.9037 18.9014 20.9837 18.1089C21.0638 17.3164 20.8467 16.5224 20.3746 15.8808C19.9025 15.2393 19.209 14.7958 18.4286 14.6365V8.78652C18.4286 7.93404 18.0899 7.11648 17.4871 6.51368C16.8843 5.91089 16.0668 5.57224 15.2143 5.57224H12.909L14.3837 4.09881C14.5044 3.9781 14.5722 3.81438 14.5722 3.64367C14.5722 3.47296 14.5044 3.30924 14.3837 3.18853C14.263 3.06782 14.0993 3 13.9286 3C13.7579 3 13.5941 3.06782 13.4734 3.18853L10.902 5.75995C10.8421 5.81967 10.7946 5.89061 10.7622 5.96871C10.7298 6.04681 10.7131 6.13054 10.7131 6.2151C10.7131 6.29965 10.7298 6.38338 10.7622 6.46148C10.7946 6.53958 10.8421 6.61052 10.902 6.67024L13.4734 9.24167ZM19.7143 17.7865C19.7143 18.0398 19.6644 18.2906 19.5675 18.5246C19.4706 18.7585 19.3285 18.9711 19.1494 19.1502C18.9703 19.3293 18.7577 19.4714 18.5237 19.5683C18.2898 19.6652 18.039 19.7151 17.7857 19.7151C17.5324 19.7151 17.2817 19.6652 17.0477 19.5683C16.8137 19.4714 16.6011 19.3293 16.422 19.1502C16.2429 18.9711 16.1009 18.7585 16.0039 18.5246C15.907 18.2906 15.8571 18.0398 15.8571 17.7865C15.8571 17.275 16.0603 16.7845 16.422 16.4228C16.7837 16.0611 17.2742 15.858 17.7857 15.858C18.2972 15.858 18.7877 16.0611 19.1494 16.4228C19.5111 16.7845 19.7143 17.275 19.7143 17.7865ZM9.42857 6.2151C9.42872 6.95609 9.17285 7.67437 8.70427 8.24839C8.23568 8.82241 7.58316 9.2169 6.85714 9.3651V15.2151C6.85714 15.7266 7.06033 16.2171 7.42201 16.5788C7.78368 16.9405 8.27422 17.1437 8.78571 17.1437H11.091L9.61628 15.6702C9.49557 15.5495 9.42776 15.3858 9.42776 15.2151C9.42776 15.0444 9.49557 14.8807 9.61628 14.76C9.73699 14.6392 9.90071 14.5714 10.0714 14.5714C10.2421 14.5714 10.4059 14.6392 10.5266 14.76L13.098 17.3314C13.1579 17.3911 13.2054 17.462 13.2378 17.5401C13.2702 17.6182 13.2869 17.702 13.2869 17.7865C13.2869 17.8711 13.2702 17.9548 13.2378 18.0329C13.2054 18.111 13.1579 18.1819 13.098 18.2417L10.5266 20.8131C10.4059 20.9338 10.2421 21.0016 10.0714 21.0016C9.90071 21.0016 9.73699 20.9338 9.61628 20.8131C9.49557 20.6924 9.42776 20.5287 9.42776 20.358C9.42776 20.1872 9.49557 20.0235 9.61628 19.9028L11.091 18.4294H8.78571C7.93323 18.4294 7.11567 18.0907 6.51287 17.4879C5.91007 16.8851 5.57143 16.0676 5.57143 15.2151V9.3651C4.97555 9.24346 4.42641 8.95528 3.98778 8.53401C3.54916 8.11274 3.23904 7.57568 3.09346 6.9852C2.94787 6.39472 2.97279 5.77506 3.16532 5.19818C3.35786 4.62129 3.7101 4.11087 4.18114 3.72619C4.65218 3.3415 5.22269 3.09835 5.82641 3.02496C6.43012 2.95157 7.04228 3.05096 7.59176 3.31159C8.14125 3.57221 8.60552 3.98338 8.93065 4.49733C9.25579 5.01129 9.42844 5.60693 9.42857 6.2151ZM8.14286 6.2151C8.14286 5.96183 8.09297 5.71105 7.99605 5.47706C7.89913 5.24308 7.75707 5.03048 7.57799 4.85139C7.39891 4.67231 7.1863 4.53025 6.95232 4.43333C6.71833 4.33641 6.46755 4.28653 6.21428 4.28653C5.96102 4.28653 5.71024 4.33641 5.47625 4.43333C5.24227 4.53025 5.02966 4.67231 4.85058 4.85139C4.6715 5.03048 4.52944 5.24308 4.43252 5.47706C4.3356 5.71105 4.28571 5.96183 4.28571 6.2151C4.28571 6.72659 4.4889 7.21713 4.85058 7.5788C5.21226 7.94048 5.7028 8.14367 6.21428 8.14367C6.72577 8.14367 7.21631 7.94048 7.57799 7.5788C7.93967 7.21713 8.14286 6.72659 8.14286 6.2151Z" fill="black" stroke="black" stroke-width="0.5" />
                                </svg>
                            </a>

                            <div class="img">
                                <img src="data/images/bike-4.jpg" alt="">
                                <!-- <div class="perks">
                                        <span class="perk new">Novinka</span>
                                        <span class="perk best">Bestseller</span>
                                        <span class="perk sale">-50%</span>
                                    </div> -->
                                <div class="hover">
                                    <a href="" class="btn full-orange add-cart"><span>Do košíku</span></a>
                                    <a href="" class="btn border-white"><span>Detail</span></a>
                                    <ul>
                                        <li>XS</li>
                                        <li>S</li>
                                        <li>M</li>
                                        <li>L</li>
                                        <li>XL</li>
                                        <li>XXL</li>
                                    </ul>
                                </div>
                            </div>
                            <div class="helper">
                                <div class="upper-text">
                                    <h3 class="small">Slash 98GX</h3>
                                    <span class="stock in">Skladem</span>
                                </div>
                                <div class="lower-text">
                                    <span class="old-cost">45 950 Kč</span>
                                    <span class="final-price">32 850 Kč</span>
                                </div>
                            </div>

                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section class="main-cat-section">
        <div class="container">

            <h2 class="mark">Hlavní kategorie</h2>
            <div class="flex-block">

                <a href="" class="category big">
                    <img src="data/images/cat-1.jpg" alt="">
                    <div class="text">
                        <h3 class="small">JÍZDNÍ KOLA</h3>
                    </div>
                </a>
                <div class="smalls">
                    <a href="" class="category">
                        <img src="data/images/cat-2.jpg" alt="">
                        <div class="text">
                            <h3 class="small">Oblečení</h3>
                        </div>
                    </a>
                    <a href="" class="category">
                        <img src="data/images/cat-3.jpg" alt="">
                        <div class="text">
                            <h3 class="small">Komponenty</h3>
                        </div>
                    </a>
                    <a href="" class="category">
                        <img src="data/images/cat-4.jpg" alt="">
                        <div class="text">
                            <h3 class="small">Doplňky</h3>
                        </div>
                    </a>
                    <a href="" class="category">
                        <img src="data/images/cat-5.jpg" alt="">
                        <div class="text">
                            <h3 class="small">Přilby a chrániče</h3>
                        </div>
                    </a>
                </div>

            </div>
        </div>
    </section>
    <section class="work-with">
        <div class="container">
            <h2 class="mark">Spolupracujeme</h2>
            <div class="grid-holder">
                <a href=""><img src="data/images/collab-1.svg" alt=""></a>
                <a href=""><img src="data/images/collab-2.svg" alt=""></a>
                <a href=""><img src="data/images/collab-3.svg" alt=""></a>
                <a href=""><img src="data/images/collab-4.svg" alt=""></a>
                <a href=""><img src="data/images/collab-5.svg" alt=""></a>
                <a href=""><img src="data/images/collab-6.svg" alt=""></a>
                <a href=""><img src="data/images/collab-7.svg" alt=""></a>
            </div>
            <div class="btn-block">
                <a href="" class="btn full-dark scroll_to"><span>A další</span></a>
            </div>
        </div>
    </section>
    <section class="action-offer">
        <div class="container">
            <h2 class="screenReaderElement">Akční nabídka</h2>
            <div class="wrapper">
                <div class="countdown">
                    <span class="deduct">-40%</span>
                    <spna class="text">AKČNÍ NABÍDKA KONČÍ ZA</spna>
                    <div class="timer">
                        <div class="block hours">
                            <span>12</span>
                            <span>hodin</span>
                        </div>
                        <div class="block min">
                            <span>42</span>
                            <span>minut</span>
                        </div>
                        <div class="block sec">
                            <span>48</span>
                            <span>vteřin</span>
                        </div>
                    </div>
                </div>
                <div class="product">
                    <div class="left">
                        <div class="rate">
                            <div class="star full"></div>
                            <div class="star full"></div>
                            <div class="star full"></div>
                            <div class="star full"></div>
                            <div class="star"></div>
                        </div>
                        <h3 class="mid">CamelBack Lahvička 0.75l</h3>
                        <span class="stock">Skladem</span>
                        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.</p>
                        <div class="price-holder">
                            <div class="price">
                                <span class="old">1 650 Kč</span>
                                <span class="new">850 Kč</span>
                            </div>
                            <a href="" class="btn full-orange add-cart"><span>Do košíku</span></a>
                        </div>
                    </div>
                    <div class="img">
                        <img src="data/images/action.jpg" alt="">
                    </div>
                </div>
                <img class="path_1" src="data/images/stopa-1.png" alt="">
                <img class="path_2" src="data/images/stopa-2.png" alt="">
            </div>
        </div>
    </section>
    <section class="access-swiper">
        <div class="container">
            <h2 class="mark">Doplňky</h2>
            <div class="prod-nav-wrapper">
                <div class="swiper-button-prev prod-prev"></div>
                <div class="swiper-button-next prod-next"></div>
            </div>
            <div class="swiper prod-swiper">

                <div class="swiper-wrapper">
                    <div class="swiper-slide">
                        <div class="swiper-holder">
                            <a href="" class="compare">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M13.4734 9.24167C13.5941 9.36238 13.7579 9.43019 13.9286 9.43019C14.0993 9.43019 14.263 9.36238 14.3837 9.24167C14.5044 9.12096 14.5722 8.95724 14.5722 8.78652C14.5722 8.61581 14.5044 8.45209 14.3837 8.33138L12.909 6.85795H15.2143C15.7258 6.85795 16.2163 7.06114 16.578 7.42282C16.9397 7.7845 17.1429 8.27504 17.1429 8.78652V14.6365C16.3624 14.7958 15.6689 15.2393 15.1968 15.8808C14.7247 16.5224 14.5076 17.3164 14.5877 18.1089C14.6678 18.9014 15.0392 19.636 15.6301 20.1702C16.221 20.7044 16.9892 21.0002 17.7857 21.0002C18.5823 21.0002 19.3504 20.7044 19.9413 20.1702C20.5322 19.636 20.9037 18.9014 20.9837 18.1089C21.0638 17.3164 20.8467 16.5224 20.3746 15.8808C19.9025 15.2393 19.209 14.7958 18.4286 14.6365V8.78652C18.4286 7.93404 18.0899 7.11648 17.4871 6.51368C16.8843 5.91089 16.0668 5.57224 15.2143 5.57224H12.909L14.3837 4.09881C14.5044 3.9781 14.5722 3.81438 14.5722 3.64367C14.5722 3.47296 14.5044 3.30924 14.3837 3.18853C14.263 3.06782 14.0993 3 13.9286 3C13.7579 3 13.5941 3.06782 13.4734 3.18853L10.902 5.75995C10.8421 5.81967 10.7946 5.89061 10.7622 5.96871C10.7298 6.04681 10.7131 6.13054 10.7131 6.2151C10.7131 6.29965 10.7298 6.38338 10.7622 6.46148C10.7946 6.53958 10.8421 6.61052 10.902 6.67024L13.4734 9.24167ZM19.7143 17.7865C19.7143 18.0398 19.6644 18.2906 19.5675 18.5246C19.4706 18.7585 19.3285 18.9711 19.1494 19.1502C18.9703 19.3293 18.7577 19.4714 18.5237 19.5683C18.2898 19.6652 18.039 19.7151 17.7857 19.7151C17.5324 19.7151 17.2817 19.6652 17.0477 19.5683C16.8137 19.4714 16.6011 19.3293 16.422 19.1502C16.2429 18.9711 16.1009 18.7585 16.0039 18.5246C15.907 18.2906 15.8571 18.0398 15.8571 17.7865C15.8571 17.275 16.0603 16.7845 16.422 16.4228C16.7837 16.0611 17.2742 15.858 17.7857 15.858C18.2972 15.858 18.7877 16.0611 19.1494 16.4228C19.5111 16.7845 19.7143 17.275 19.7143 17.7865ZM9.42857 6.2151C9.42872 6.95609 9.17285 7.67437 8.70427 8.24839C8.23568 8.82241 7.58316 9.2169 6.85714 9.3651V15.2151C6.85714 15.7266 7.06033 16.2171 7.42201 16.5788C7.78368 16.9405 8.27422 17.1437 8.78571 17.1437H11.091L9.61628 15.6702C9.49557 15.5495 9.42776 15.3858 9.42776 15.2151C9.42776 15.0444 9.49557 14.8807 9.61628 14.76C9.73699 14.6392 9.90071 14.5714 10.0714 14.5714C10.2421 14.5714 10.4059 14.6392 10.5266 14.76L13.098 17.3314C13.1579 17.3911 13.2054 17.462 13.2378 17.5401C13.2702 17.6182 13.2869 17.702 13.2869 17.7865C13.2869 17.8711 13.2702 17.9548 13.2378 18.0329C13.2054 18.111 13.1579 18.1819 13.098 18.2417L10.5266 20.8131C10.4059 20.9338 10.2421 21.0016 10.0714 21.0016C9.90071 21.0016 9.73699 20.9338 9.61628 20.8131C9.49557 20.6924 9.42776 20.5287 9.42776 20.358C9.42776 20.1872 9.49557 20.0235 9.61628 19.9028L11.091 18.4294H8.78571C7.93323 18.4294 7.11567 18.0907 6.51287 17.4879C5.91007 16.8851 5.57143 16.0676 5.57143 15.2151V9.3651C4.97555 9.24346 4.42641 8.95528 3.98778 8.53401C3.54916 8.11274 3.23904 7.57568 3.09346 6.9852C2.94787 6.39472 2.97279 5.77506 3.16532 5.19818C3.35786 4.62129 3.7101 4.11087 4.18114 3.72619C4.65218 3.3415 5.22269 3.09835 5.82641 3.02496C6.43012 2.95157 7.04228 3.05096 7.59176 3.31159C8.14125 3.57221 8.60552 3.98338 8.93065 4.49733C9.25579 5.01129 9.42844 5.60693 9.42857 6.2151ZM8.14286 6.2151C8.14286 5.96183 8.09297 5.71105 7.99605 5.47706C7.89913 5.24308 7.75707 5.03048 7.57799 4.85139C7.39891 4.67231 7.1863 4.53025 6.95232 4.43333C6.71833 4.33641 6.46755 4.28653 6.21428 4.28653C5.96102 4.28653 5.71024 4.33641 5.47625 4.43333C5.24227 4.53025 5.02966 4.67231 4.85058 4.85139C4.6715 5.03048 4.52944 5.24308 4.43252 5.47706C4.3356 5.71105 4.28571 5.96183 4.28571 6.2151C4.28571 6.72659 4.4889 7.21713 4.85058 7.5788C5.21226 7.94048 5.7028 8.14367 6.21428 8.14367C6.72577 8.14367 7.21631 7.94048 7.57799 7.5788C7.93967 7.21713 8.14286 6.72659 8.14286 6.2151Z" fill="black" stroke="black" stroke-width="0.5" />
                                </svg>
                            </a>

                            <div class="img">
                                <img src="data/images/access-1.jpg" alt="">
                                <!-- <div class="perks">
                                        <span class="perk new">Novinka</span>
                                        <span class="perk best">Bestseller</span>
                                        <span class="perk sale">-50%</span>
                                    </div> -->
                                <div class="hover">
                                    <a href="" class="btn full-orange add-cart"><span>Do košíku</span></a>
                                    <a href="" class="btn border-white"><span>Detail</span></a>
                                    <ul>
                                        <li>XS</li>
                                        <li>S</li>
                                        <li>M</li>
                                        <li>L</li>
                                        <li>XL</li>
                                        <li>XXL</li>
                                    </ul>
                                </div>
                            </div>
                            <div class="helper">
                                <div class="upper-text">
                                    <h3 class="small">Fenix 6 Pro</h3>
                                    <span class="stock in">Skladem</span>
                                </div>
                                <div class="lower-text">
                                    <span class="old-cost">950</span>
                                    <span class="final-price">16 850 Kč</span>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="swiper-holder">
                            <a href="" class="compare">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M13.4734 9.24167C13.5941 9.36238 13.7579 9.43019 13.9286 9.43019C14.0993 9.43019 14.263 9.36238 14.3837 9.24167C14.5044 9.12096 14.5722 8.95724 14.5722 8.78652C14.5722 8.61581 14.5044 8.45209 14.3837 8.33138L12.909 6.85795H15.2143C15.7258 6.85795 16.2163 7.06114 16.578 7.42282C16.9397 7.7845 17.1429 8.27504 17.1429 8.78652V14.6365C16.3624 14.7958 15.6689 15.2393 15.1968 15.8808C14.7247 16.5224 14.5076 17.3164 14.5877 18.1089C14.6678 18.9014 15.0392 19.636 15.6301 20.1702C16.221 20.7044 16.9892 21.0002 17.7857 21.0002C18.5823 21.0002 19.3504 20.7044 19.9413 20.1702C20.5322 19.636 20.9037 18.9014 20.9837 18.1089C21.0638 17.3164 20.8467 16.5224 20.3746 15.8808C19.9025 15.2393 19.209 14.7958 18.4286 14.6365V8.78652C18.4286 7.93404 18.0899 7.11648 17.4871 6.51368C16.8843 5.91089 16.0668 5.57224 15.2143 5.57224H12.909L14.3837 4.09881C14.5044 3.9781 14.5722 3.81438 14.5722 3.64367C14.5722 3.47296 14.5044 3.30924 14.3837 3.18853C14.263 3.06782 14.0993 3 13.9286 3C13.7579 3 13.5941 3.06782 13.4734 3.18853L10.902 5.75995C10.8421 5.81967 10.7946 5.89061 10.7622 5.96871C10.7298 6.04681 10.7131 6.13054 10.7131 6.2151C10.7131 6.29965 10.7298 6.38338 10.7622 6.46148C10.7946 6.53958 10.8421 6.61052 10.902 6.67024L13.4734 9.24167ZM19.7143 17.7865C19.7143 18.0398 19.6644 18.2906 19.5675 18.5246C19.4706 18.7585 19.3285 18.9711 19.1494 19.1502C18.9703 19.3293 18.7577 19.4714 18.5237 19.5683C18.2898 19.6652 18.039 19.7151 17.7857 19.7151C17.5324 19.7151 17.2817 19.6652 17.0477 19.5683C16.8137 19.4714 16.6011 19.3293 16.422 19.1502C16.2429 18.9711 16.1009 18.7585 16.0039 18.5246C15.907 18.2906 15.8571 18.0398 15.8571 17.7865C15.8571 17.275 16.0603 16.7845 16.422 16.4228C16.7837 16.0611 17.2742 15.858 17.7857 15.858C18.2972 15.858 18.7877 16.0611 19.1494 16.4228C19.5111 16.7845 19.7143 17.275 19.7143 17.7865ZM9.42857 6.2151C9.42872 6.95609 9.17285 7.67437 8.70427 8.24839C8.23568 8.82241 7.58316 9.2169 6.85714 9.3651V15.2151C6.85714 15.7266 7.06033 16.2171 7.42201 16.5788C7.78368 16.9405 8.27422 17.1437 8.78571 17.1437H11.091L9.61628 15.6702C9.49557 15.5495 9.42776 15.3858 9.42776 15.2151C9.42776 15.0444 9.49557 14.8807 9.61628 14.76C9.73699 14.6392 9.90071 14.5714 10.0714 14.5714C10.2421 14.5714 10.4059 14.6392 10.5266 14.76L13.098 17.3314C13.1579 17.3911 13.2054 17.462 13.2378 17.5401C13.2702 17.6182 13.2869 17.702 13.2869 17.7865C13.2869 17.8711 13.2702 17.9548 13.2378 18.0329C13.2054 18.111 13.1579 18.1819 13.098 18.2417L10.5266 20.8131C10.4059 20.9338 10.2421 21.0016 10.0714 21.0016C9.90071 21.0016 9.73699 20.9338 9.61628 20.8131C9.49557 20.6924 9.42776 20.5287 9.42776 20.358C9.42776 20.1872 9.49557 20.0235 9.61628 19.9028L11.091 18.4294H8.78571C7.93323 18.4294 7.11567 18.0907 6.51287 17.4879C5.91007 16.8851 5.57143 16.0676 5.57143 15.2151V9.3651C4.97555 9.24346 4.42641 8.95528 3.98778 8.53401C3.54916 8.11274 3.23904 7.57568 3.09346 6.9852C2.94787 6.39472 2.97279 5.77506 3.16532 5.19818C3.35786 4.62129 3.7101 4.11087 4.18114 3.72619C4.65218 3.3415 5.22269 3.09835 5.82641 3.02496C6.43012 2.95157 7.04228 3.05096 7.59176 3.31159C8.14125 3.57221 8.60552 3.98338 8.93065 4.49733C9.25579 5.01129 9.42844 5.60693 9.42857 6.2151ZM8.14286 6.2151C8.14286 5.96183 8.09297 5.71105 7.99605 5.47706C7.89913 5.24308 7.75707 5.03048 7.57799 4.85139C7.39891 4.67231 7.1863 4.53025 6.95232 4.43333C6.71833 4.33641 6.46755 4.28653 6.21428 4.28653C5.96102 4.28653 5.71024 4.33641 5.47625 4.43333C5.24227 4.53025 5.02966 4.67231 4.85058 4.85139C4.6715 5.03048 4.52944 5.24308 4.43252 5.47706C4.3356 5.71105 4.28571 5.96183 4.28571 6.2151C4.28571 6.72659 4.4889 7.21713 4.85058 7.5788C5.21226 7.94048 5.7028 8.14367 6.21428 8.14367C6.72577 8.14367 7.21631 7.94048 7.57799 7.5788C7.93967 7.21713 8.14286 6.72659 8.14286 6.2151Z" fill="black" stroke="black" stroke-width="0.5" />
                                </svg>
                            </a>

                            <div class="img">
                                <img src="data/images/access-2.jpg" alt="">
                                <!-- <div class="perks">
                                        <span class="perk new">Novinka</span>
                                        <span class="perk best">Bestseller</span>
                                        <span class="perk sale">-50%</span>
                                    </div> -->
                                <div class="hover">
                                    <a href="" class="btn full-orange add-cart"><span>Do košíku</span></a>
                                    <a href="" class="btn border-white"><span>Detail</span></a>
                                    <ul>
                                        <li>XS</li>
                                        <li>S</li>
                                        <li>M</li>
                                        <li>L</li>
                                        <li>XL</li>
                                        <li>XXL</li>
                                    </ul>
                                </div>
                            </div>
                            <div class="helper">
                                <div class="upper-text">
                                    <h3 class="small">Vacuum Stainless 1l</h3>
                                    <span class="stock in">Skladem</span>
                                </div>
                                <div class="lower-text">
                                    <span class="old-cost">950</span>
                                    <span class="final-price">16 850 Kč</span>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="swiper-holder">
                            <a href="" class="compare">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M13.4734 9.24167C13.5941 9.36238 13.7579 9.43019 13.9286 9.43019C14.0993 9.43019 14.263 9.36238 14.3837 9.24167C14.5044 9.12096 14.5722 8.95724 14.5722 8.78652C14.5722 8.61581 14.5044 8.45209 14.3837 8.33138L12.909 6.85795H15.2143C15.7258 6.85795 16.2163 7.06114 16.578 7.42282C16.9397 7.7845 17.1429 8.27504 17.1429 8.78652V14.6365C16.3624 14.7958 15.6689 15.2393 15.1968 15.8808C14.7247 16.5224 14.5076 17.3164 14.5877 18.1089C14.6678 18.9014 15.0392 19.636 15.6301 20.1702C16.221 20.7044 16.9892 21.0002 17.7857 21.0002C18.5823 21.0002 19.3504 20.7044 19.9413 20.1702C20.5322 19.636 20.9037 18.9014 20.9837 18.1089C21.0638 17.3164 20.8467 16.5224 20.3746 15.8808C19.9025 15.2393 19.209 14.7958 18.4286 14.6365V8.78652C18.4286 7.93404 18.0899 7.11648 17.4871 6.51368C16.8843 5.91089 16.0668 5.57224 15.2143 5.57224H12.909L14.3837 4.09881C14.5044 3.9781 14.5722 3.81438 14.5722 3.64367C14.5722 3.47296 14.5044 3.30924 14.3837 3.18853C14.263 3.06782 14.0993 3 13.9286 3C13.7579 3 13.5941 3.06782 13.4734 3.18853L10.902 5.75995C10.8421 5.81967 10.7946 5.89061 10.7622 5.96871C10.7298 6.04681 10.7131 6.13054 10.7131 6.2151C10.7131 6.29965 10.7298 6.38338 10.7622 6.46148C10.7946 6.53958 10.8421 6.61052 10.902 6.67024L13.4734 9.24167ZM19.7143 17.7865C19.7143 18.0398 19.6644 18.2906 19.5675 18.5246C19.4706 18.7585 19.3285 18.9711 19.1494 19.1502C18.9703 19.3293 18.7577 19.4714 18.5237 19.5683C18.2898 19.6652 18.039 19.7151 17.7857 19.7151C17.5324 19.7151 17.2817 19.6652 17.0477 19.5683C16.8137 19.4714 16.6011 19.3293 16.422 19.1502C16.2429 18.9711 16.1009 18.7585 16.0039 18.5246C15.907 18.2906 15.8571 18.0398 15.8571 17.7865C15.8571 17.275 16.0603 16.7845 16.422 16.4228C16.7837 16.0611 17.2742 15.858 17.7857 15.858C18.2972 15.858 18.7877 16.0611 19.1494 16.4228C19.5111 16.7845 19.7143 17.275 19.7143 17.7865ZM9.42857 6.2151C9.42872 6.95609 9.17285 7.67437 8.70427 8.24839C8.23568 8.82241 7.58316 9.2169 6.85714 9.3651V15.2151C6.85714 15.7266 7.06033 16.2171 7.42201 16.5788C7.78368 16.9405 8.27422 17.1437 8.78571 17.1437H11.091L9.61628 15.6702C9.49557 15.5495 9.42776 15.3858 9.42776 15.2151C9.42776 15.0444 9.49557 14.8807 9.61628 14.76C9.73699 14.6392 9.90071 14.5714 10.0714 14.5714C10.2421 14.5714 10.4059 14.6392 10.5266 14.76L13.098 17.3314C13.1579 17.3911 13.2054 17.462 13.2378 17.5401C13.2702 17.6182 13.2869 17.702 13.2869 17.7865C13.2869 17.8711 13.2702 17.9548 13.2378 18.0329C13.2054 18.111 13.1579 18.1819 13.098 18.2417L10.5266 20.8131C10.4059 20.9338 10.2421 21.0016 10.0714 21.0016C9.90071 21.0016 9.73699 20.9338 9.61628 20.8131C9.49557 20.6924 9.42776 20.5287 9.42776 20.358C9.42776 20.1872 9.49557 20.0235 9.61628 19.9028L11.091 18.4294H8.78571C7.93323 18.4294 7.11567 18.0907 6.51287 17.4879C5.91007 16.8851 5.57143 16.0676 5.57143 15.2151V9.3651C4.97555 9.24346 4.42641 8.95528 3.98778 8.53401C3.54916 8.11274 3.23904 7.57568 3.09346 6.9852C2.94787 6.39472 2.97279 5.77506 3.16532 5.19818C3.35786 4.62129 3.7101 4.11087 4.18114 3.72619C4.65218 3.3415 5.22269 3.09835 5.82641 3.02496C6.43012 2.95157 7.04228 3.05096 7.59176 3.31159C8.14125 3.57221 8.60552 3.98338 8.93065 4.49733C9.25579 5.01129 9.42844 5.60693 9.42857 6.2151ZM8.14286 6.2151C8.14286 5.96183 8.09297 5.71105 7.99605 5.47706C7.89913 5.24308 7.75707 5.03048 7.57799 4.85139C7.39891 4.67231 7.1863 4.53025 6.95232 4.43333C6.71833 4.33641 6.46755 4.28653 6.21428 4.28653C5.96102 4.28653 5.71024 4.33641 5.47625 4.43333C5.24227 4.53025 5.02966 4.67231 4.85058 4.85139C4.6715 5.03048 4.52944 5.24308 4.43252 5.47706C4.3356 5.71105 4.28571 5.96183 4.28571 6.2151C4.28571 6.72659 4.4889 7.21713 4.85058 7.5788C5.21226 7.94048 5.7028 8.14367 6.21428 8.14367C6.72577 8.14367 7.21631 7.94048 7.57799 7.5788C7.93967 7.21713 8.14286 6.72659 8.14286 6.2151Z" fill="black" stroke="black" stroke-width="0.5" />
                                </svg>
                            </a>

                            <div class="img">
                                <img src="data/images/access-3.jpg" alt="">
                                <!-- <div class="perks">
                                        <span class="perk new">Novinka</span>
                                        <span class="perk best">Bestseller</span>
                                        <span class="perk sale">-50%</span>
                                    </div> -->
                                <div class="hover">
                                    <a href="" class="btn full-orange add-cart"><span>Do košíku</span></a>
                                    <a href="" class="btn border-white"><span>Detail</span></a>
                                    <ul>
                                        <li>XS</li>
                                        <li>S</li>
                                        <li>M</li>
                                        <li>L</li>
                                        <li>XL</li>
                                        <li>XXL</li>
                                    </ul>
                                </div>
                            </div>
                            <div class="helper">
                                <div class="upper-text">
                                    <h3 class="small">AeroComfort Triathlon 3.0 TSA</h3>
                                    <span class="stock in">Skladem</span>
                                </div>
                                <div class="lower-text">
                                    <span class="old-cost">950</span>
                                    <span class="final-price">16 850 Kč</span>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="swiper-holder">
                            <a href="" class="compare">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M13.4734 9.24167C13.5941 9.36238 13.7579 9.43019 13.9286 9.43019C14.0993 9.43019 14.263 9.36238 14.3837 9.24167C14.5044 9.12096 14.5722 8.95724 14.5722 8.78652C14.5722 8.61581 14.5044 8.45209 14.3837 8.33138L12.909 6.85795H15.2143C15.7258 6.85795 16.2163 7.06114 16.578 7.42282C16.9397 7.7845 17.1429 8.27504 17.1429 8.78652V14.6365C16.3624 14.7958 15.6689 15.2393 15.1968 15.8808C14.7247 16.5224 14.5076 17.3164 14.5877 18.1089C14.6678 18.9014 15.0392 19.636 15.6301 20.1702C16.221 20.7044 16.9892 21.0002 17.7857 21.0002C18.5823 21.0002 19.3504 20.7044 19.9413 20.1702C20.5322 19.636 20.9037 18.9014 20.9837 18.1089C21.0638 17.3164 20.8467 16.5224 20.3746 15.8808C19.9025 15.2393 19.209 14.7958 18.4286 14.6365V8.78652C18.4286 7.93404 18.0899 7.11648 17.4871 6.51368C16.8843 5.91089 16.0668 5.57224 15.2143 5.57224H12.909L14.3837 4.09881C14.5044 3.9781 14.5722 3.81438 14.5722 3.64367C14.5722 3.47296 14.5044 3.30924 14.3837 3.18853C14.263 3.06782 14.0993 3 13.9286 3C13.7579 3 13.5941 3.06782 13.4734 3.18853L10.902 5.75995C10.8421 5.81967 10.7946 5.89061 10.7622 5.96871C10.7298 6.04681 10.7131 6.13054 10.7131 6.2151C10.7131 6.29965 10.7298 6.38338 10.7622 6.46148C10.7946 6.53958 10.8421 6.61052 10.902 6.67024L13.4734 9.24167ZM19.7143 17.7865C19.7143 18.0398 19.6644 18.2906 19.5675 18.5246C19.4706 18.7585 19.3285 18.9711 19.1494 19.1502C18.9703 19.3293 18.7577 19.4714 18.5237 19.5683C18.2898 19.6652 18.039 19.7151 17.7857 19.7151C17.5324 19.7151 17.2817 19.6652 17.0477 19.5683C16.8137 19.4714 16.6011 19.3293 16.422 19.1502C16.2429 18.9711 16.1009 18.7585 16.0039 18.5246C15.907 18.2906 15.8571 18.0398 15.8571 17.7865C15.8571 17.275 16.0603 16.7845 16.422 16.4228C16.7837 16.0611 17.2742 15.858 17.7857 15.858C18.2972 15.858 18.7877 16.0611 19.1494 16.4228C19.5111 16.7845 19.7143 17.275 19.7143 17.7865ZM9.42857 6.2151C9.42872 6.95609 9.17285 7.67437 8.70427 8.24839C8.23568 8.82241 7.58316 9.2169 6.85714 9.3651V15.2151C6.85714 15.7266 7.06033 16.2171 7.42201 16.5788C7.78368 16.9405 8.27422 17.1437 8.78571 17.1437H11.091L9.61628 15.6702C9.49557 15.5495 9.42776 15.3858 9.42776 15.2151C9.42776 15.0444 9.49557 14.8807 9.61628 14.76C9.73699 14.6392 9.90071 14.5714 10.0714 14.5714C10.2421 14.5714 10.4059 14.6392 10.5266 14.76L13.098 17.3314C13.1579 17.3911 13.2054 17.462 13.2378 17.5401C13.2702 17.6182 13.2869 17.702 13.2869 17.7865C13.2869 17.8711 13.2702 17.9548 13.2378 18.0329C13.2054 18.111 13.1579 18.1819 13.098 18.2417L10.5266 20.8131C10.4059 20.9338 10.2421 21.0016 10.0714 21.0016C9.90071 21.0016 9.73699 20.9338 9.61628 20.8131C9.49557 20.6924 9.42776 20.5287 9.42776 20.358C9.42776 20.1872 9.49557 20.0235 9.61628 19.9028L11.091 18.4294H8.78571C7.93323 18.4294 7.11567 18.0907 6.51287 17.4879C5.91007 16.8851 5.57143 16.0676 5.57143 15.2151V9.3651C4.97555 9.24346 4.42641 8.95528 3.98778 8.53401C3.54916 8.11274 3.23904 7.57568 3.09346 6.9852C2.94787 6.39472 2.97279 5.77506 3.16532 5.19818C3.35786 4.62129 3.7101 4.11087 4.18114 3.72619C4.65218 3.3415 5.22269 3.09835 5.82641 3.02496C6.43012 2.95157 7.04228 3.05096 7.59176 3.31159C8.14125 3.57221 8.60552 3.98338 8.93065 4.49733C9.25579 5.01129 9.42844 5.60693 9.42857 6.2151ZM8.14286 6.2151C8.14286 5.96183 8.09297 5.71105 7.99605 5.47706C7.89913 5.24308 7.75707 5.03048 7.57799 4.85139C7.39891 4.67231 7.1863 4.53025 6.95232 4.43333C6.71833 4.33641 6.46755 4.28653 6.21428 4.28653C5.96102 4.28653 5.71024 4.33641 5.47625 4.43333C5.24227 4.53025 5.02966 4.67231 4.85058 4.85139C4.6715 5.03048 4.52944 5.24308 4.43252 5.47706C4.3356 5.71105 4.28571 5.96183 4.28571 6.2151C4.28571 6.72659 4.4889 7.21713 4.85058 7.5788C5.21226 7.94048 5.7028 8.14367 6.21428 8.14367C6.72577 8.14367 7.21631 7.94048 7.57799 7.5788C7.93967 7.21713 8.14286 6.72659 8.14286 6.2151Z" fill="black" stroke="black" stroke-width="0.5" />
                                </svg>
                            </a>

                            <div class="img">
                                <img src="data/images/access-4.jpg" alt="">
                                <!-- <div class="perks">
                                        <span class="perk new">Novinka</span>
                                        <span class="perk best">Bestseller</span>
                                        <span class="perk sale">-50%</span>
                                    </div> -->
                                <div class="hover">
                                    <a href="" class="btn full-orange add-cart"><span>Do košíku</span></a>
                                    <a href="" class="btn border-white"><span>Detail</span></a>
                                    <ul>
                                        <li>XS</li>
                                        <li>S</li>
                                        <li>M</li>
                                        <li>L</li>
                                        <li>XL</li>
                                        <li>XXL</li>
                                    </ul>
                                </div>
                            </div>
                            <div class="helper">
                                <div class="upper-text">
                                    <h3 class="small">MET přilba PARACHUTE MCR MIPS</h3>
                                    <span class="stock in">Skladem</span>
                                </div>
                                <div class="lower-text">
                                    <span class="old-cost">950</span>
                                    <span class="final-price">16 850 Kč</span>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="swiper-holder">
                            <a href="" class="compare">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M13.4734 9.24167C13.5941 9.36238 13.7579 9.43019 13.9286 9.43019C14.0993 9.43019 14.263 9.36238 14.3837 9.24167C14.5044 9.12096 14.5722 8.95724 14.5722 8.78652C14.5722 8.61581 14.5044 8.45209 14.3837 8.33138L12.909 6.85795H15.2143C15.7258 6.85795 16.2163 7.06114 16.578 7.42282C16.9397 7.7845 17.1429 8.27504 17.1429 8.78652V14.6365C16.3624 14.7958 15.6689 15.2393 15.1968 15.8808C14.7247 16.5224 14.5076 17.3164 14.5877 18.1089C14.6678 18.9014 15.0392 19.636 15.6301 20.1702C16.221 20.7044 16.9892 21.0002 17.7857 21.0002C18.5823 21.0002 19.3504 20.7044 19.9413 20.1702C20.5322 19.636 20.9037 18.9014 20.9837 18.1089C21.0638 17.3164 20.8467 16.5224 20.3746 15.8808C19.9025 15.2393 19.209 14.7958 18.4286 14.6365V8.78652C18.4286 7.93404 18.0899 7.11648 17.4871 6.51368C16.8843 5.91089 16.0668 5.57224 15.2143 5.57224H12.909L14.3837 4.09881C14.5044 3.9781 14.5722 3.81438 14.5722 3.64367C14.5722 3.47296 14.5044 3.30924 14.3837 3.18853C14.263 3.06782 14.0993 3 13.9286 3C13.7579 3 13.5941 3.06782 13.4734 3.18853L10.902 5.75995C10.8421 5.81967 10.7946 5.89061 10.7622 5.96871C10.7298 6.04681 10.7131 6.13054 10.7131 6.2151C10.7131 6.29965 10.7298 6.38338 10.7622 6.46148C10.7946 6.53958 10.8421 6.61052 10.902 6.67024L13.4734 9.24167ZM19.7143 17.7865C19.7143 18.0398 19.6644 18.2906 19.5675 18.5246C19.4706 18.7585 19.3285 18.9711 19.1494 19.1502C18.9703 19.3293 18.7577 19.4714 18.5237 19.5683C18.2898 19.6652 18.039 19.7151 17.7857 19.7151C17.5324 19.7151 17.2817 19.6652 17.0477 19.5683C16.8137 19.4714 16.6011 19.3293 16.422 19.1502C16.2429 18.9711 16.1009 18.7585 16.0039 18.5246C15.907 18.2906 15.8571 18.0398 15.8571 17.7865C15.8571 17.275 16.0603 16.7845 16.422 16.4228C16.7837 16.0611 17.2742 15.858 17.7857 15.858C18.2972 15.858 18.7877 16.0611 19.1494 16.4228C19.5111 16.7845 19.7143 17.275 19.7143 17.7865ZM9.42857 6.2151C9.42872 6.95609 9.17285 7.67437 8.70427 8.24839C8.23568 8.82241 7.58316 9.2169 6.85714 9.3651V15.2151C6.85714 15.7266 7.06033 16.2171 7.42201 16.5788C7.78368 16.9405 8.27422 17.1437 8.78571 17.1437H11.091L9.61628 15.6702C9.49557 15.5495 9.42776 15.3858 9.42776 15.2151C9.42776 15.0444 9.49557 14.8807 9.61628 14.76C9.73699 14.6392 9.90071 14.5714 10.0714 14.5714C10.2421 14.5714 10.4059 14.6392 10.5266 14.76L13.098 17.3314C13.1579 17.3911 13.2054 17.462 13.2378 17.5401C13.2702 17.6182 13.2869 17.702 13.2869 17.7865C13.2869 17.8711 13.2702 17.9548 13.2378 18.0329C13.2054 18.111 13.1579 18.1819 13.098 18.2417L10.5266 20.8131C10.4059 20.9338 10.2421 21.0016 10.0714 21.0016C9.90071 21.0016 9.73699 20.9338 9.61628 20.8131C9.49557 20.6924 9.42776 20.5287 9.42776 20.358C9.42776 20.1872 9.49557 20.0235 9.61628 19.9028L11.091 18.4294H8.78571C7.93323 18.4294 7.11567 18.0907 6.51287 17.4879C5.91007 16.8851 5.57143 16.0676 5.57143 15.2151V9.3651C4.97555 9.24346 4.42641 8.95528 3.98778 8.53401C3.54916 8.11274 3.23904 7.57568 3.09346 6.9852C2.94787 6.39472 2.97279 5.77506 3.16532 5.19818C3.35786 4.62129 3.7101 4.11087 4.18114 3.72619C4.65218 3.3415 5.22269 3.09835 5.82641 3.02496C6.43012 2.95157 7.04228 3.05096 7.59176 3.31159C8.14125 3.57221 8.60552 3.98338 8.93065 4.49733C9.25579 5.01129 9.42844 5.60693 9.42857 6.2151ZM8.14286 6.2151C8.14286 5.96183 8.09297 5.71105 7.99605 5.47706C7.89913 5.24308 7.75707 5.03048 7.57799 4.85139C7.39891 4.67231 7.1863 4.53025 6.95232 4.43333C6.71833 4.33641 6.46755 4.28653 6.21428 4.28653C5.96102 4.28653 5.71024 4.33641 5.47625 4.43333C5.24227 4.53025 5.02966 4.67231 4.85058 4.85139C4.6715 5.03048 4.52944 5.24308 4.43252 5.47706C4.3356 5.71105 4.28571 5.96183 4.28571 6.2151C4.28571 6.72659 4.4889 7.21713 4.85058 7.5788C5.21226 7.94048 5.7028 8.14367 6.21428 8.14367C6.72577 8.14367 7.21631 7.94048 7.57799 7.5788C7.93967 7.21713 8.14286 6.72659 8.14286 6.2151Z" fill="black" stroke="black" stroke-width="0.5" />
                                </svg>
                            </a>

                            <div class="img">
                                <img src="data/images/access-1.jpg" alt="">
                                <!-- <div class="perks">
                                        <span class="perk new">Novinka</span>
                                        <span class="perk best">Bestseller</span>
                                        <span class="perk sale">-50%</span>
                                    </div> -->
                                <div class="hover">
                                    <a href="" class="btn full-orange add-cart"><span>Do košíku</span></a>
                                    <a href="" class="btn border-white"><span>Detail</span></a>
                                    <ul>
                                        <li>XS</li>
                                        <li>S</li>
                                        <li>M</li>
                                        <li>L</li>
                                        <li>XL</li>
                                        <li>XXL</li>
                                    </ul>
                                </div>
                            </div>
                            <div class="helper">
                                <div class="upper-text">
                                    <h3 class="small">Fenix 6 Pro</h3>
                                    <span class="stock in">Skladem</span>
                                </div>
                                <div class="lower-text">
                                    <span class="old-cost">950</span>
                                    <span class="final-price">16 850 Kč</span>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="swiper-holder">
                            <a href="" class="compare">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M13.4734 9.24167C13.5941 9.36238 13.7579 9.43019 13.9286 9.43019C14.0993 9.43019 14.263 9.36238 14.3837 9.24167C14.5044 9.12096 14.5722 8.95724 14.5722 8.78652C14.5722 8.61581 14.5044 8.45209 14.3837 8.33138L12.909 6.85795H15.2143C15.7258 6.85795 16.2163 7.06114 16.578 7.42282C16.9397 7.7845 17.1429 8.27504 17.1429 8.78652V14.6365C16.3624 14.7958 15.6689 15.2393 15.1968 15.8808C14.7247 16.5224 14.5076 17.3164 14.5877 18.1089C14.6678 18.9014 15.0392 19.636 15.6301 20.1702C16.221 20.7044 16.9892 21.0002 17.7857 21.0002C18.5823 21.0002 19.3504 20.7044 19.9413 20.1702C20.5322 19.636 20.9037 18.9014 20.9837 18.1089C21.0638 17.3164 20.8467 16.5224 20.3746 15.8808C19.9025 15.2393 19.209 14.7958 18.4286 14.6365V8.78652C18.4286 7.93404 18.0899 7.11648 17.4871 6.51368C16.8843 5.91089 16.0668 5.57224 15.2143 5.57224H12.909L14.3837 4.09881C14.5044 3.9781 14.5722 3.81438 14.5722 3.64367C14.5722 3.47296 14.5044 3.30924 14.3837 3.18853C14.263 3.06782 14.0993 3 13.9286 3C13.7579 3 13.5941 3.06782 13.4734 3.18853L10.902 5.75995C10.8421 5.81967 10.7946 5.89061 10.7622 5.96871C10.7298 6.04681 10.7131 6.13054 10.7131 6.2151C10.7131 6.29965 10.7298 6.38338 10.7622 6.46148C10.7946 6.53958 10.8421 6.61052 10.902 6.67024L13.4734 9.24167ZM19.7143 17.7865C19.7143 18.0398 19.6644 18.2906 19.5675 18.5246C19.4706 18.7585 19.3285 18.9711 19.1494 19.1502C18.9703 19.3293 18.7577 19.4714 18.5237 19.5683C18.2898 19.6652 18.039 19.7151 17.7857 19.7151C17.5324 19.7151 17.2817 19.6652 17.0477 19.5683C16.8137 19.4714 16.6011 19.3293 16.422 19.1502C16.2429 18.9711 16.1009 18.7585 16.0039 18.5246C15.907 18.2906 15.8571 18.0398 15.8571 17.7865C15.8571 17.275 16.0603 16.7845 16.422 16.4228C16.7837 16.0611 17.2742 15.858 17.7857 15.858C18.2972 15.858 18.7877 16.0611 19.1494 16.4228C19.5111 16.7845 19.7143 17.275 19.7143 17.7865ZM9.42857 6.2151C9.42872 6.95609 9.17285 7.67437 8.70427 8.24839C8.23568 8.82241 7.58316 9.2169 6.85714 9.3651V15.2151C6.85714 15.7266 7.06033 16.2171 7.42201 16.5788C7.78368 16.9405 8.27422 17.1437 8.78571 17.1437H11.091L9.61628 15.6702C9.49557 15.5495 9.42776 15.3858 9.42776 15.2151C9.42776 15.0444 9.49557 14.8807 9.61628 14.76C9.73699 14.6392 9.90071 14.5714 10.0714 14.5714C10.2421 14.5714 10.4059 14.6392 10.5266 14.76L13.098 17.3314C13.1579 17.3911 13.2054 17.462 13.2378 17.5401C13.2702 17.6182 13.2869 17.702 13.2869 17.7865C13.2869 17.8711 13.2702 17.9548 13.2378 18.0329C13.2054 18.111 13.1579 18.1819 13.098 18.2417L10.5266 20.8131C10.4059 20.9338 10.2421 21.0016 10.0714 21.0016C9.90071 21.0016 9.73699 20.9338 9.61628 20.8131C9.49557 20.6924 9.42776 20.5287 9.42776 20.358C9.42776 20.1872 9.49557 20.0235 9.61628 19.9028L11.091 18.4294H8.78571C7.93323 18.4294 7.11567 18.0907 6.51287 17.4879C5.91007 16.8851 5.57143 16.0676 5.57143 15.2151V9.3651C4.97555 9.24346 4.42641 8.95528 3.98778 8.53401C3.54916 8.11274 3.23904 7.57568 3.09346 6.9852C2.94787 6.39472 2.97279 5.77506 3.16532 5.19818C3.35786 4.62129 3.7101 4.11087 4.18114 3.72619C4.65218 3.3415 5.22269 3.09835 5.82641 3.02496C6.43012 2.95157 7.04228 3.05096 7.59176 3.31159C8.14125 3.57221 8.60552 3.98338 8.93065 4.49733C9.25579 5.01129 9.42844 5.60693 9.42857 6.2151ZM8.14286 6.2151C8.14286 5.96183 8.09297 5.71105 7.99605 5.47706C7.89913 5.24308 7.75707 5.03048 7.57799 4.85139C7.39891 4.67231 7.1863 4.53025 6.95232 4.43333C6.71833 4.33641 6.46755 4.28653 6.21428 4.28653C5.96102 4.28653 5.71024 4.33641 5.47625 4.43333C5.24227 4.53025 5.02966 4.67231 4.85058 4.85139C4.6715 5.03048 4.52944 5.24308 4.43252 5.47706C4.3356 5.71105 4.28571 5.96183 4.28571 6.2151C4.28571 6.72659 4.4889 7.21713 4.85058 7.5788C5.21226 7.94048 5.7028 8.14367 6.21428 8.14367C6.72577 8.14367 7.21631 7.94048 7.57799 7.5788C7.93967 7.21713 8.14286 6.72659 8.14286 6.2151Z" fill="black" stroke="black" stroke-width="0.5" />
                                </svg>
                            </a>

                            <div class="img">
                                <img src="data/images/access-2.jpg" alt="">
                                <!-- <div class="perks">
                                        <span class="perk new">Novinka</span>
                                        <span class="perk best">Bestseller</span>
                                        <span class="perk sale">-50%</span>
                                    </div> -->
                                <div class="hover">
                                    <a href="" class="btn full-orange add-cart"><span>Do košíku</span></a>
                                    <a href="" class="btn border-white"><span>Detail</span></a>
                                    <ul>
                                        <li>XS</li>
                                        <li>S</li>
                                        <li>M</li>
                                        <li>L</li>
                                        <li>XL</li>
                                        <li>XXL</li>
                                    </ul>
                                </div>
                            </div>
                            <div class="helper">
                                <div class="upper-text">
                                    <h3 class="small">Vacuum Stainless 1l</h3>
                                    <span class="stock in">Skladem</span>
                                </div>
                                <div class="lower-text">
                                    <span class="old-cost">950</span>
                                    <span class="final-price">16 850 Kč</span>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="swiper-holder">
                            <a href="" class="compare">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M13.4734 9.24167C13.5941 9.36238 13.7579 9.43019 13.9286 9.43019C14.0993 9.43019 14.263 9.36238 14.3837 9.24167C14.5044 9.12096 14.5722 8.95724 14.5722 8.78652C14.5722 8.61581 14.5044 8.45209 14.3837 8.33138L12.909 6.85795H15.2143C15.7258 6.85795 16.2163 7.06114 16.578 7.42282C16.9397 7.7845 17.1429 8.27504 17.1429 8.78652V14.6365C16.3624 14.7958 15.6689 15.2393 15.1968 15.8808C14.7247 16.5224 14.5076 17.3164 14.5877 18.1089C14.6678 18.9014 15.0392 19.636 15.6301 20.1702C16.221 20.7044 16.9892 21.0002 17.7857 21.0002C18.5823 21.0002 19.3504 20.7044 19.9413 20.1702C20.5322 19.636 20.9037 18.9014 20.9837 18.1089C21.0638 17.3164 20.8467 16.5224 20.3746 15.8808C19.9025 15.2393 19.209 14.7958 18.4286 14.6365V8.78652C18.4286 7.93404 18.0899 7.11648 17.4871 6.51368C16.8843 5.91089 16.0668 5.57224 15.2143 5.57224H12.909L14.3837 4.09881C14.5044 3.9781 14.5722 3.81438 14.5722 3.64367C14.5722 3.47296 14.5044 3.30924 14.3837 3.18853C14.263 3.06782 14.0993 3 13.9286 3C13.7579 3 13.5941 3.06782 13.4734 3.18853L10.902 5.75995C10.8421 5.81967 10.7946 5.89061 10.7622 5.96871C10.7298 6.04681 10.7131 6.13054 10.7131 6.2151C10.7131 6.29965 10.7298 6.38338 10.7622 6.46148C10.7946 6.53958 10.8421 6.61052 10.902 6.67024L13.4734 9.24167ZM19.7143 17.7865C19.7143 18.0398 19.6644 18.2906 19.5675 18.5246C19.4706 18.7585 19.3285 18.9711 19.1494 19.1502C18.9703 19.3293 18.7577 19.4714 18.5237 19.5683C18.2898 19.6652 18.039 19.7151 17.7857 19.7151C17.5324 19.7151 17.2817 19.6652 17.0477 19.5683C16.8137 19.4714 16.6011 19.3293 16.422 19.1502C16.2429 18.9711 16.1009 18.7585 16.0039 18.5246C15.907 18.2906 15.8571 18.0398 15.8571 17.7865C15.8571 17.275 16.0603 16.7845 16.422 16.4228C16.7837 16.0611 17.2742 15.858 17.7857 15.858C18.2972 15.858 18.7877 16.0611 19.1494 16.4228C19.5111 16.7845 19.7143 17.275 19.7143 17.7865ZM9.42857 6.2151C9.42872 6.95609 9.17285 7.67437 8.70427 8.24839C8.23568 8.82241 7.58316 9.2169 6.85714 9.3651V15.2151C6.85714 15.7266 7.06033 16.2171 7.42201 16.5788C7.78368 16.9405 8.27422 17.1437 8.78571 17.1437H11.091L9.61628 15.6702C9.49557 15.5495 9.42776 15.3858 9.42776 15.2151C9.42776 15.0444 9.49557 14.8807 9.61628 14.76C9.73699 14.6392 9.90071 14.5714 10.0714 14.5714C10.2421 14.5714 10.4059 14.6392 10.5266 14.76L13.098 17.3314C13.1579 17.3911 13.2054 17.462 13.2378 17.5401C13.2702 17.6182 13.2869 17.702 13.2869 17.7865C13.2869 17.8711 13.2702 17.9548 13.2378 18.0329C13.2054 18.111 13.1579 18.1819 13.098 18.2417L10.5266 20.8131C10.4059 20.9338 10.2421 21.0016 10.0714 21.0016C9.90071 21.0016 9.73699 20.9338 9.61628 20.8131C9.49557 20.6924 9.42776 20.5287 9.42776 20.358C9.42776 20.1872 9.49557 20.0235 9.61628 19.9028L11.091 18.4294H8.78571C7.93323 18.4294 7.11567 18.0907 6.51287 17.4879C5.91007 16.8851 5.57143 16.0676 5.57143 15.2151V9.3651C4.97555 9.24346 4.42641 8.95528 3.98778 8.53401C3.54916 8.11274 3.23904 7.57568 3.09346 6.9852C2.94787 6.39472 2.97279 5.77506 3.16532 5.19818C3.35786 4.62129 3.7101 4.11087 4.18114 3.72619C4.65218 3.3415 5.22269 3.09835 5.82641 3.02496C6.43012 2.95157 7.04228 3.05096 7.59176 3.31159C8.14125 3.57221 8.60552 3.98338 8.93065 4.49733C9.25579 5.01129 9.42844 5.60693 9.42857 6.2151ZM8.14286 6.2151C8.14286 5.96183 8.09297 5.71105 7.99605 5.47706C7.89913 5.24308 7.75707 5.03048 7.57799 4.85139C7.39891 4.67231 7.1863 4.53025 6.95232 4.43333C6.71833 4.33641 6.46755 4.28653 6.21428 4.28653C5.96102 4.28653 5.71024 4.33641 5.47625 4.43333C5.24227 4.53025 5.02966 4.67231 4.85058 4.85139C4.6715 5.03048 4.52944 5.24308 4.43252 5.47706C4.3356 5.71105 4.28571 5.96183 4.28571 6.2151C4.28571 6.72659 4.4889 7.21713 4.85058 7.5788C5.21226 7.94048 5.7028 8.14367 6.21428 8.14367C6.72577 8.14367 7.21631 7.94048 7.57799 7.5788C7.93967 7.21713 8.14286 6.72659 8.14286 6.2151Z" fill="black" stroke="black" stroke-width="0.5" />
                                </svg>
                            </a>

                            <div class="img">
                                <img src="data/images/access-3.jpg" alt="">
                                <!-- <div class="perks">
                                        <span class="perk new">Novinka</span>
                                        <span class="perk best">Bestseller</span>
                                        <span class="perk sale">-50%</span>
                                    </div> -->
                                <div class="hover">
                                    <a href="" class="btn full-orange add-cart"><span>Do košíku</span></a>
                                    <a href="" class="btn border-white"><span>Detail</span></a>
                                    <ul>
                                        <li>XS</li>
                                        <li>S</li>
                                        <li>M</li>
                                        <li>L</li>
                                        <li>XL</li>
                                        <li>XXL</li>
                                    </ul>
                                </div>
                            </div>
                            <div class="helper">
                                <div class="upper-text">
                                    <h3 class="small">AeroComfort Triathlon 3.0 TSA</h3>
                                    <span class="stock in">Skladem</span>
                                </div>
                                <div class="lower-text">
                                    <span class="old-cost">950</span>
                                    <span class="final-price">16 850 Kč</span>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="swiper-holder">
                            <a href="" class="compare">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M13.4734 9.24167C13.5941 9.36238 13.7579 9.43019 13.9286 9.43019C14.0993 9.43019 14.263 9.36238 14.3837 9.24167C14.5044 9.12096 14.5722 8.95724 14.5722 8.78652C14.5722 8.61581 14.5044 8.45209 14.3837 8.33138L12.909 6.85795H15.2143C15.7258 6.85795 16.2163 7.06114 16.578 7.42282C16.9397 7.7845 17.1429 8.27504 17.1429 8.78652V14.6365C16.3624 14.7958 15.6689 15.2393 15.1968 15.8808C14.7247 16.5224 14.5076 17.3164 14.5877 18.1089C14.6678 18.9014 15.0392 19.636 15.6301 20.1702C16.221 20.7044 16.9892 21.0002 17.7857 21.0002C18.5823 21.0002 19.3504 20.7044 19.9413 20.1702C20.5322 19.636 20.9037 18.9014 20.9837 18.1089C21.0638 17.3164 20.8467 16.5224 20.3746 15.8808C19.9025 15.2393 19.209 14.7958 18.4286 14.6365V8.78652C18.4286 7.93404 18.0899 7.11648 17.4871 6.51368C16.8843 5.91089 16.0668 5.57224 15.2143 5.57224H12.909L14.3837 4.09881C14.5044 3.9781 14.5722 3.81438 14.5722 3.64367C14.5722 3.47296 14.5044 3.30924 14.3837 3.18853C14.263 3.06782 14.0993 3 13.9286 3C13.7579 3 13.5941 3.06782 13.4734 3.18853L10.902 5.75995C10.8421 5.81967 10.7946 5.89061 10.7622 5.96871C10.7298 6.04681 10.7131 6.13054 10.7131 6.2151C10.7131 6.29965 10.7298 6.38338 10.7622 6.46148C10.7946 6.53958 10.8421 6.61052 10.902 6.67024L13.4734 9.24167ZM19.7143 17.7865C19.7143 18.0398 19.6644 18.2906 19.5675 18.5246C19.4706 18.7585 19.3285 18.9711 19.1494 19.1502C18.9703 19.3293 18.7577 19.4714 18.5237 19.5683C18.2898 19.6652 18.039 19.7151 17.7857 19.7151C17.5324 19.7151 17.2817 19.6652 17.0477 19.5683C16.8137 19.4714 16.6011 19.3293 16.422 19.1502C16.2429 18.9711 16.1009 18.7585 16.0039 18.5246C15.907 18.2906 15.8571 18.0398 15.8571 17.7865C15.8571 17.275 16.0603 16.7845 16.422 16.4228C16.7837 16.0611 17.2742 15.858 17.7857 15.858C18.2972 15.858 18.7877 16.0611 19.1494 16.4228C19.5111 16.7845 19.7143 17.275 19.7143 17.7865ZM9.42857 6.2151C9.42872 6.95609 9.17285 7.67437 8.70427 8.24839C8.23568 8.82241 7.58316 9.2169 6.85714 9.3651V15.2151C6.85714 15.7266 7.06033 16.2171 7.42201 16.5788C7.78368 16.9405 8.27422 17.1437 8.78571 17.1437H11.091L9.61628 15.6702C9.49557 15.5495 9.42776 15.3858 9.42776 15.2151C9.42776 15.0444 9.49557 14.8807 9.61628 14.76C9.73699 14.6392 9.90071 14.5714 10.0714 14.5714C10.2421 14.5714 10.4059 14.6392 10.5266 14.76L13.098 17.3314C13.1579 17.3911 13.2054 17.462 13.2378 17.5401C13.2702 17.6182 13.2869 17.702 13.2869 17.7865C13.2869 17.8711 13.2702 17.9548 13.2378 18.0329C13.2054 18.111 13.1579 18.1819 13.098 18.2417L10.5266 20.8131C10.4059 20.9338 10.2421 21.0016 10.0714 21.0016C9.90071 21.0016 9.73699 20.9338 9.61628 20.8131C9.49557 20.6924 9.42776 20.5287 9.42776 20.358C9.42776 20.1872 9.49557 20.0235 9.61628 19.9028L11.091 18.4294H8.78571C7.93323 18.4294 7.11567 18.0907 6.51287 17.4879C5.91007 16.8851 5.57143 16.0676 5.57143 15.2151V9.3651C4.97555 9.24346 4.42641 8.95528 3.98778 8.53401C3.54916 8.11274 3.23904 7.57568 3.09346 6.9852C2.94787 6.39472 2.97279 5.77506 3.16532 5.19818C3.35786 4.62129 3.7101 4.11087 4.18114 3.72619C4.65218 3.3415 5.22269 3.09835 5.82641 3.02496C6.43012 2.95157 7.04228 3.05096 7.59176 3.31159C8.14125 3.57221 8.60552 3.98338 8.93065 4.49733C9.25579 5.01129 9.42844 5.60693 9.42857 6.2151ZM8.14286 6.2151C8.14286 5.96183 8.09297 5.71105 7.99605 5.47706C7.89913 5.24308 7.75707 5.03048 7.57799 4.85139C7.39891 4.67231 7.1863 4.53025 6.95232 4.43333C6.71833 4.33641 6.46755 4.28653 6.21428 4.28653C5.96102 4.28653 5.71024 4.33641 5.47625 4.43333C5.24227 4.53025 5.02966 4.67231 4.85058 4.85139C4.6715 5.03048 4.52944 5.24308 4.43252 5.47706C4.3356 5.71105 4.28571 5.96183 4.28571 6.2151C4.28571 6.72659 4.4889 7.21713 4.85058 7.5788C5.21226 7.94048 5.7028 8.14367 6.21428 8.14367C6.72577 8.14367 7.21631 7.94048 7.57799 7.5788C7.93967 7.21713 8.14286 6.72659 8.14286 6.2151Z" fill="black" stroke="black" stroke-width="0.5" />
                                </svg>
                            </a>

                            <div class="img">
                                <img src="data/images/access-4.jpg" alt="">
                                <!-- <div class="perks">
                                        <span class="perk new">Novinka</span>
                                        <span class="perk best">Bestseller</span>
                                        <span class="perk sale">-50%</span>
                                    </div> -->
                                <div class="hover">
                                    <a href="" class="btn full-orange add-cart"><span>Do košíku</span></a>
                                    <a href="" class="btn border-white"><span>Detail</span></a>
                                    <ul>
                                        <li>XS</li>
                                        <li>S</li>
                                        <li>M</li>
                                        <li>L</li>
                                        <li>XL</li>
                                        <li>XXL</li>
                                    </ul>
                                </div>
                            </div>
                            <div class="helper">
                                <div class="upper-text">
                                    <h3 class="small">MET přilba PARACHUTE MCR MIPS</h3>
                                    <span class="stock in">Skladem</span>
                                </div>
                                <div class="lower-text">
                                    <span class="old-cost">950</span>
                                    <span class="final-price">16 850 Kč</span>
                                </div>
                            </div>

                        </div>
                    </div>
                </div>
            </div>
            <div class="btn-holder">
                <a href="" class="btn full-dark"><span>Zobrazit sekci doplňky</span></a>
            </div>
        </div>
    </section>
    <section class="chose-bike">
        <div class="container">
            <div class="flex-holder">
                <img src="data/images/chose-bike.jpg" alt="">
                <div class="text">
                    <h2>Jak správně vybrat kolo</h2>
                    <p>Ve 5 jednoduchých krocích zjistite jaké kolo by pro Vás bylo nejlepší v našem jednoduchém průvodci, lorem ipsum dolor sit amet.</p>
                    <span class="w"><strong>V jakém terénu budete jezdit?</strong></span>
                    <a href="" class="btn border-white"><span>Spustit průvodce</span></a>
                </div>
            </div>
        </div>
    </section>
    <section class="why-us">
        <div class="container">
            <div class="flex-block">
                <div class="left">
                    <h2 class="mark">Proč nakupovat u nás</h2>
                    <p>Prodejem vybavení nejen pro cyklisty se zabíváme už více jak 15 let, consectetur adipiscing elit, sed do eiusmod tempor didunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea mmodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.</p>
                    <a href="" class="btn full-dark"><span>Více informací o nás</span></a>
                </div>
                <div class="right">
                    <div class="perk">
                        <img src="data/images/why-us-1.svg" alt="">
                        <span>Obrovský výběr <br>
                            cyklistického sortimentu</span>
                    </div>
                    <div class="perk">
                        <img src="data/images/why-us-2.svg" alt="">
                        <span>15 let na trhu</span>
                    </div>
                    <div class="perk">
                        <img src="data/images/why-us-3.svg" alt="">
                        <span>u vás doma <br>
                            do 2 dnů</span>
                    </div>
                    <div class="perk">
                        <img src="data/images/why-us-4.svg" alt="">
                        <span>Doprava zdarma <br>
                            od 1000 Kč</span>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section class="join-club">
        <div class="container">
            <img src="data/images/Vectora.png" alt="" class="left">
            <img src="data/images/Vectora.png" alt="" class="right">
            <div class="flex-block">
                <h2>STAŇTE SE ČLENEM NAŠEHO <br>
                    VIP KLUBU</h2>
                <p>Consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. </p>
                <ul>
                    <li>Automatická sleva 5% na celou objednávku</li>
                    <li>Dodatečné dárky k nákupu</li>
                    <li>Detailní informace o objednávce</li>
                    <li>Možnost účastnit se soutěží o hodnotné ceny</li>
                    <li>Aktuální informace o novém či akčním sortimentu</li>
                    <li>možnost bezplatné výměny zboží</li>
                </ul>
                <div class="btn-block">
                    <a href="" class="btn full-dark hov"><span>Registrovat se / Přihlásit</span></a>
                    <a href="" class="btn border-white"><span>Více informací</span></a>
                </div>
            </div>
        </div>
    </section>
    <section class="blog-posts">
        <div class="container">
            <h2 class="mark">BLOGOVÉ ČLÁNKY</h2>
            <div class="grid-block">
                <a href="" class="post">
                    <div class="img">
                        <img src="data/images/blog-prew-1.jpg" alt="">
                        <span class="date">16. 04. 2022</span>
                    </div>
                    <div class="text">
                        <h3>Jaký plášť na pořádny dirt</h3>
                        <p>Consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. </p>
                        <span>Přečíst článek</span>
                    </div>
                </a>
                <a href="" class="post">
                    <div class="img">
                        <img src="data/images/blog-prew-2.jpg" alt="">
                        <span class="date">16. 04. 2022</span>
                    </div>
                    <div class="text">
                        <h3>Rozšiřujeme rodinu našich značek o CANOYON</h3>
                        <p>Consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. </p>
                        <span>Přečíst článek</span>
                    </div>
                </a>
                <a href="" class="post">
                    <div class="img">
                        <img src="data/images/blog-prew-3.jpg" alt="">
                        <span class="date">16. 04. 2022</span>
                    </div>
                    <div class="text">
                        <h3>Oblečení pro ranní mrazíky a odpoleční výheň</h3>
                        <p>Consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. </p>
                        <span>Přečíst článek</span>
                    </div>
                </a>
                <a href="" class="post">
                    <div class="img">
                        <img src="data/images/blog-prew-4.jpg" alt="">
                        <span class="date">16. 04. 2022</span>
                    </div>
                    <div class="text">
                        <h3>Lorem ipsum dolor sit amet adipscing lorem ipsum</h3>
                        <p>Consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. </p>
                        <span>Přečíst článek</span>
                    </div>
                </a>
            </div>
            <div class="btn-block">
                <a href="" class="btn full-dark"><span>Zobrazit blog</span></a>
            </div>
        </div>
    </section>
</main>
<?php include("footer.php") ?>