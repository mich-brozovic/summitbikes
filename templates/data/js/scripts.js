...
let lastScrollTop = 0;
const delta = 10;
const triggerPosition = window.innerHeight * 2; // získá výšku viewportu
...
$(window).scroll(function () {
  ...
  if (st > lastScrollTop && st > triggerPosition) {
...
jQuery(document).ready(function ($) {
  ...
  $(document).ready(() => {
    ...
    if (gall.length > 0) {
      const firstImageSrc = gall
        .find(".images-holder .img img:first")
        .attr("src");
  ...
  $(document).on("click", ".switcher", function (e) {
    ...
    mainBlock
      .find(".content-part[data-target='" + id + "']")
      .addClass("active")
      .siblings()
      .removeClass("active");
  });
  ...
  $(document).on("click", ".close", function (e) {
    ...
    $(".categories-navigation").removeClass("active");
    $(".categories-navigation .category ul").removeClass("active");
    $(document).find(".mobile-class").removeClass("active");
  ...
  $(document).on("click", "#categories-dropdownd", function (e) {
    ...
    const ul = li.find("ul");
    $(document).find(".mobile-class").toggleClass("active");
  ...
  $(document).click(function (event) {
    ...
    if (
      !$(formElement).is(event.target) &&
      $(formElement).has(event.target).length === 0
    ) {
  ...
  $(document).on(
    "click",
    ".link-part .grid-block .block .menu-title",
    function (e) {
      const t = $(this);
      const windowWidth = $(window).width();
  ...
  if (windowWidth <= 1023) {
    $(".categories-navigation .icon").click(function (event) {
      event.preventDefault();
      const cats = $(this).closest(".category").find("ul");
  ...
  $(".bg-categroy").click(function (e) {
    if (
      $(".categories-navigation").hasClass("active-hov") ||
      $(".category").hasClass("seen")
    ) {
  ...
  $(document).ready(() => {
    ...
    if (windowWidth >= 1460) {
      const updatePadding = () => {
        const marginLeft = $(".container").css("margin-left");
        const paddingLeft = $(".container").css("padding-left");

        const totalPadding = parseInt(marginLeft) + parseInt(paddingLeft);

        $(".categories-navigation .holder").css({
          "padding-left": totalPadding + "px",
          "padding-right": totalPadding + "px",
        });
      };
